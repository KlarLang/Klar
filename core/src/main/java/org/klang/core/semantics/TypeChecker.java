 ...
 * * <h3>Passes:</h3>
 * <ol>
 * <li><strong>Collection:</strong> Scans function signatures to populate the Symbol Table.</li>
 * <li><strong>Verification:</strong> Traverses function bodies and statements to check types.</li>
 * </ol>
...
public class TypeChecker {
    ...
    public TypeSymbol checkCallExpression(CallExpressionNode node, TypeContext ctx) {
        ...
        if (!functions.contains(funcName)) {
            semanticError(DiagnosticCode.E217, "Undefined function '" + funcName + "'", "Import module or check spelling", null, node.callee);
        }

        FunctionSymbol fn = functions.resolve(funcName);

        if (node.arguments.size() != fn.parameters.size()) {
            semanticError(DiagnosticCode.E208, "Argument count mismatch", "Expected " + fn.parameters.size() + ", got " + node.arguments.size(), null, node);
        }
        ...
        for (int i = 0; i < node.arguments.size(); i++) {
            ...
            if (!isAssignable(arg, param)) {
                semanticError(DiagnosticCode.E207, "Type mismatch in argument " + (i + 1), "Expected " + param + ", got " + arg, null, node.arguments.get(i));
            }
        }
    ...
    public void checkStatement(StatementNode node, TypeContext ctx) {
        if (node instanceof ConstantDeclarationNode c) {
            ...
            if (!isAssignable(value, declared)) semanticError(DiagnosticCode.E207, "Type mismatch", "Cannot assign " + value + " to constant " + declared, null, node);
            if (!isCompileTimeConstant(c.value)) semanticError(DiagnosticCode.E213, "Constant must be evaluable at compile-time", "Use literals", null, node);
        ...
        if (node instanceof VariableDeclarationNode v) {
            ...
            if (!isAssignable(value, declared)) semanticError(DiagnosticCode.E207, "Type mismatch", "Cannot assign " + value + " to " + declared, null, node);
            ctx.declare(v.name.getValue(), declared);
            return;
        }
        if (node instanceof AssignmentStatementNode a) {
            TypeSymbol target = resolveTarget(a.value, ctx);
            if (target instanceof ConstantSymbol) semanticError(DiagnosticCode.E209, "Cannot assign to constant", "Remove assignment", null, node);
            TypeSymbol value = checkExpression(a.value, ctx, ExpressionContext.ASSIGNMENT);
            if (!isAssignable(value, target)) semanticError(DiagnosticCode.E207, "Type mismatch", "Expected " + target + ", got " + value, null, node);
            return;
        }
        if (node instanceof BlockStatementNode b) {
            TypeContext local = new TypeContext(ctx);
            for (StatementNode stmt : b.statements) checkStatement(stmt, local);
            return;
        }
        if (node instanceof ExpressionStatementNode e) { checkExpression(e.expression, ctx, ExpressionContext.GENERAL); return; }
        if (node instanceof FunctionDeclarationNode f) { checkFunctionDeclaration(f, ctx); return; }
        if (node instanceof ReturnStatementNode r) { checkReturn(r, ctx); return; }
        if (node instanceof DecisionStatementNode d) { checkDecision(d, ctx); return; }
        if (node instanceof WhileStatementNode w) {
             TypeSymbol c = checkExpression(w.condition, ctx, ExpressionContext.CONDITION);
             if (!isBoolean(c)) semanticError(DiagnosticCode.E211, "Condition must be boolean", null, null, w);
             checkStatement(w.body, new TypeContext(ctx));
             return;
        }
        if (node instanceof ModuleDeclarationNode || node instanceof ImportDeclarationNode) return;
    ...
    public void checkDecision(DecisionStatementNode d, TypeContext ctx) {
        ...
        if (!isBoolean(c)) semanticError(DiagnosticCode.E211, "If condition must be boolean", null, null, d);
        ...
        if (d.afterallBlock != null) checkStatement(d.afterallBlock, new TypeContext(ctx));
    }

    public void checkFunctionDeclaration(FunctionDeclarationNode node, TypeContext ctx) {
        if (node.name.getValue().equals("main")) {
            if (!node.parameters.isEmpty()) semanticError(DiagnosticCode.E210, "Main cannot have parameters", null, null, node);
            TypeSymbol ret = resolveTypeSymbol(node.returnType);
            if (!(ret instanceof PrimitiveTypeSymbol p && p.type == Type.VOID)) semanticError(DiagnosticCode.E210, "Main must return void", null, null, node);
        }
        currentReturnType = resolveTypeSymbol(node.returnType);
        TypeContext local = new TypeContext(ctx);
        for (ParameterNode param : node.parameters) local.declare(param.name.getValue(), resolveTypeSymbol(param.type));
    ...
    public void checkReturn(ReturnStatementNode node, TypeContext ctx) {
        ...
        if (currentReturnType instanceof PrimitiveTypeSymbol p && p.type == Type.VOID) {
            if (node.value != null) semanticError(DiagnosticCode.E207, "Void function cannot return value", null, null, node);
            return;
        }
        if (node.value == null) semanticError(DiagnosticCode.E205, "Must return a value", null, null, node);
        TypeSymbol val = checkExpression(node.value, ctx, ExpressionContext.RETURN);
        if (!isAssignable(val, currentReturnType)) semanticError(DiagnosticCode.E207, "Return type mismatch", "Expected " + currentReturnType, null, node);
    }
    ...
    public TypeSymbol checkExpression(ExpressionNode node, TypeContext ctx, ExpressionContext context) {
        if (node instanceof LiteralExpressionNode l) {
            if (l.value.getType() == TokenType.INTEGER_LITERAL){
                ...
                boolean isAllowed = context == ExpressionContext.ASSIGNMENT || context == ExpressionContext.GENERAL || v.equals("0") || v.equals("1");
                if (!isAllowed) semanticError(DiagnosticCode.E212, "Magic Number '" + v + "' violation", "Use constants", null, node);
                return new PrimitiveTypeSymbol(Type.INTEGER, true);
            }
            if (l.value.getType() == TokenType.DOUBLE_LITERAL) {
                if (context != ExpressionContext.ASSIGNMENT && context != ExpressionContext.RETURN) semanticError(DiagnosticCode.E212, "Decimal literal context error", "Assign to variable", null, node);
        ...
        if (node instanceof VariableExpressionNode v) {
            ...
            if (s == null) semanticError(DiagnosticCode.E217, "Unresolved variable '" + v.name.getValue() + "'", null, null, v);
            return s;
        }
        if (node instanceof BinaryExpressionNode b) return checkBinary(b, ctx);
        if (node instanceof CallExpressionNode c) return checkCallExpression(c, ctx);
        if (node instanceof IndexExpressionNode i) {
            ...
            if (!(t instanceof ArrayTypeSymbol a)) { semanticError(DiagnosticCode.E215, "Not an array", null, null, i.target); return null; }
            if (!(idx instanceof PrimitiveTypeSymbol p && p.type == Type.INTEGER)) semanticError(DiagnosticCode.E207, "Index must be integer", null, null, i.index);
    ...
    public TypeSymbol checkBinary(BinaryExpressionNode node, TypeContext ctx) {
        ...
        if ((left instanceof PrimitiveTypeSymbol p && p.type == Type.UNKNOWN) || (right instanceof PrimitiveTypeSymbol p2 && p2.type == Type.UNKNOWN)) return new PrimitiveTypeSymbol(Type.UNKNOWN, false);

        switch (node.operator.getType()) {
            case PLUS: return (left.isString() || right.isString()) ? new PrimitiveTypeSymbol(Type.STRING, true) : (left.isDouble() || right.isDouble() ? new PrimitiveTypeSymbol(Type.DOUBLE, true) : new PrimitiveTypeSymbol(Type.INTEGER, true));
            case MINUS: case MULTIPLY: case DIVISION: return (left.isDouble() || right.isDouble()) ? new PrimitiveTypeSymbol(Type.DOUBLE, true) : new PrimitiveTypeSymbol(Type.INTEGER, true);
            case DOUBLEEQUAL: case NOTEQUAL: case GT: case LT: case GTE: case LTE: case AND: case OR: return new PrimitiveTypeSymbol(Type.BOOLEAN, true);
            default: semanticError(DiagnosticCode.E214, "Invalid operator", null, null, node); return new PrimitiveTypeSymbol(Type.UNKNOWN, false);
        }
    }
    ...
    public TypeSymbol resolveTypeSymbol(TypeReferenceNode node) {
        Type base = switch (node.getBaseType().getType()) {
            case INTEGER_TYPE -> Type.INTEGER; case DOUBLE_TYPE -> Type.DOUBLE;
            case BOOLEAN_TYPE -> Type.BOOLEAN; case STRING_TYPE -> Type.STRING;
            case CHARACTER_TYPE -> Type.CHARACTER; case VOID -> Type.VOID;
            default -> Type.UNKNOWN;
        };
        if (base == Type.UNKNOWN) semanticError(DiagnosticCode.E201, "Unknown type", null, null, node);
    ...
    public boolean isAssignable(TypeSymbol from, TypeSymbol to) {
        if (to instanceof PrimitiveTypeSymbol p && p.type == Type.UNKNOWN) return true;
        if (from instanceof PrimitiveTypeSymbol p && p.type == Type.UNKNOWN) return true;
        if (from.isInteger() && to.isDouble()) return true;
        return to.isAssignableFrom(from);
    }

    private boolean isBoolean(TypeSymbol t) { return t instanceof PrimitiveTypeSymbol p && p.type == Type.BOOLEAN; }
    private boolean isCompileTimeConstant(ExpressionNode node) { return node instanceof LiteralExpressionNode || (node instanceof BinaryExpressionNode b && isCompileTimeConstant(b.left) && isCompileTimeConstant(b.right)); }
    
    private TypeSymbol resolveTarget(ExpressionNode node, TypeContext ctx) {
        if (node instanceof VariableExpressionNode v) return ctx.resolve(v.name.getValue());
        if (node instanceof IndexExpressionNode i) { checkExpression(i, ctx, ExpressionContext.ASSIGNMENT); return new PrimitiveTypeSymbol(Type.UNKNOWN, true); }
